...
wait_for_server() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if curl -sf "${CELLAR_SERVER_URL}/health" > /dev/null 2>&1; then
...
# Create a test token via the admin API
create_test_token() {
    # For now, generate a placeholder token
    # In production, this would call the server's token API
    # The server should be configured to accept this token or auto-create one
    echo "test-token-$(openssl rand -hex 16)"
}
