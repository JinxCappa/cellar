...
# Test 4: Invalid chunk position
log_info "Testing invalid chunk position..."
...
if echo "$SESSION" | jq -e '.upload_id' > /dev/null 2>&1; then
    ...
    # Try to upload to invalid position (e.g., position 999)
    INVALID_POS_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
        -H "Authorization: Bearer $CELLAR_TEST_TOKEN" \
        -H "Content-Type: application/octet-stream" \
        -X PUT --data-binary "test" \
        "${CELLAR_SERVER_URL}/v1/uploads/$UPLOAD_ID/chunks/999")
    log_info "Invalid chunk position status: $INVALID_POS_STATUS"
fi
...
UNAUTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -d '{"store_path": "/nix/store/test", "nar_hash": "sha256:test", "nar_size": 100}' \
    "${CELLAR_SERVER_URL}/v1/uploads")
...
INVALID_TOKEN_STATUS=$(http_get_status "/v1/uploads" "invalid-token-12345")
...
STORE_PATH="/nix/store/$(random_string 32)-duplicate-test"
