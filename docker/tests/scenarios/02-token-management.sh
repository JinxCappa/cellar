...
if echo "$TOKEN_RESPONSE" | jq -e '.token' > /dev/null 2>&1; then
    ...
    # Test 3: Use the new token
    log_info "Testing new token authentication..."
    AUTH_STATUS=$(http_get_status "/health" "$NEW_TOKEN")
    ...
    REVOKE_STATUS=$(http_post_status "/v1/admin/tokens/$TOKEN_ID/revoke" '{}' 2>/dev/null || echo "404")
    if [ "$REVOKE_STATUS" = "200" ] || [ "$REVOKE_STATUS" = "204" ]; then
        ...
        sleep 1
        REVOKED_STATUS=$(http_get_status "/v1/uploads" "$NEW_TOKEN")
