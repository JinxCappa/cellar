...
pub trait StorePathRepo: Send + Sync {
    ...
    /// If cache_id is None, returns any matching store path (admin use only).
    ...
    /// If cache_id is None, checks globally (admin/public cache use only).
    ...
    /// If cache_id is None, returns all store paths for the manifest (admin use).
