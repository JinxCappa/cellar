...
pub fn verify_narinfo(
    ...
    for sig in &narinfo.signatures {
        ...
        if let Some((_, pubkey)) = trusted_keys.iter().find(|(name, _)| name == &sig.key_name) {
            if verify_signature(narinfo, sig, pubkey).is_ok() {
                return Ok(true);
            }
        }
    }
...
mod tests {
    ...
    fn test_sign_and_verify() {
        ...
        let nar_hash = NarHash::from_sri("sha256-LCa0a2j/xo/5m0U8HTBBNBNCLXBkg7+g+YpeiGJm564=").unwrap();
    ...
    fn test_verify_narinfo_with_trusted_keys() {
        ...
        let trusted_keys = vec![
            ("trusted-cache-1".to_string(), signer.public_key().clone()),
        ];
