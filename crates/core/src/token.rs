...
mod tests {
    ...
    #[test]
    fn test_scope_parse() {
        assert_eq!(TokenScope::parse("cache:read").unwrap(), TokenScope::CacheRead);
        assert_eq!(TokenScope::parse("cache:write").unwrap(), TokenScope::CacheWrite);
        assert_eq!(TokenScope::parse("cache:admin").unwrap(), TokenScope::CacheAdmin);
