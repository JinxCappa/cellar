...
/// Skip test if Docker is not available or SKIP_POSTGRES_TESTS is set.
fn should_skip() -> bool {
    std::env::var("SKIP_POSTGRES_TESTS").is_ok()
}
...
async fn test_postgres_upload_session_lifecycle() {
    ...
    let metadata = PostgresTestMetadata::new()
        .await
        .expect("Failed to create PostgreSQL metadata");
    ...
    let retrieved = store.get_session(upload_id).await.expect("Get session failed");
...
async fn test_postgres_chunk_operations() {
    ...
    store.increment_refcount(&hash1).await.expect("Increment failed");
    store.increment_refcount(&hash1).await.expect("Increment failed");

    let chunk = store.get_chunk(&hash1).await.expect("Get failed").expect("Not found");
    assert_eq!(chunk.refcount, 2);

    store.decrement_refcount(&hash1).await.expect("Decrement failed");
    let chunk = store.get_chunk(&hash1).await.expect("Get failed").expect("Not found");
    assert_eq!(chunk.refcount, 1);

    // Test touch_chunk
    store.touch_chunk(&hash1, now).await.expect("Touch failed");
    let chunk = store.get_chunk(&hash1).await.expect("Get failed").expect("Not found");
    assert!(chunk.last_accessed_at.is_some());

    // Test delete_chunk
    store.delete_chunk(&hash2).await.expect("Delete failed");
    assert!(!store.chunk_exists(&hash2).await.expect("Check failed"));
}
...
async fn test_postgres_manifest_operations() {
    ...
    assert!(store.manifest_exists(&manifest_hash).await.expect("Check failed"));
    ...
    let manifests = store
        .get_manifests_using_chunk(&chunk_hashes[0])
        .await
...
async fn test_postgres_store_path_operations() {
    ...
    assert!(store.store_path_visible(None, &path_hash).await.expect("Check failed"));
...
async fn test_postgres_token_operations() {
    ...
    assert_eq!(retrieved.description, Some("PostgreSQL Test Token".to_string()));
    ...
    store
        ...
        .await
        .expect("Touch failed");
    let retrieved = store.get_token(token_id).await.expect("Get failed").expect("Not found");
...
async fn test_postgres_gc_job_operations() {
    ...
    // Update to running
    store
        .update_gc_job_state(job_id, "running", Some(now), None)
        .await
        .expect("Update state failed");
    ...
    store
        .update_gc_job_state(job_id, "finished", Some(now), Some(stats))
        .await
        .expect("Update state failed");

    let retrieved = store.get_gc_job(job_id).await.expect("Get failed").expect("Not found");
...
async fn test_postgres_chunk_stats() {
    ...
    store
        .increment_refcount(&test_hash("pg-stats-chunk-0"))
        .await
        .expect("Inc failed");
    store
        .increment_refcount(&test_hash("pg-stats-chunk-1"))
        .await
        .expect("Inc failed");
...
async fn test_postgres_signing_key_operations() {
    ...
    store.create_signing_key(&key).await.expect("Create signing key failed");
