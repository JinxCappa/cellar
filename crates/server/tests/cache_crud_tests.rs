...
async fn test_create_default_cache() {
    ...
    // Create a public cache with is_default=true
    let (status, response) = json_request(
        &server.router,
        "POST",
        "/v1/admin/caches",
...
async fn test_default_cache_conflict() {
    ...
    // Create first default cache
    let (status, _) = json_request(
        &server.router,
        "POST",
        "/v1/admin/caches",
        Some(json!({
            "cache_name": "first-default-cache",
            "is_public": true,
            "is_default": true
        })),
        Some(&admin_token),
    ...
    // Try to create second default cache - should conflict
    let (status, response) = json_request(
        &server.router,
        "POST",
        "/v1/admin/caches",
        Some(json!({
            "cache_name": "second-default-cache",
            "is_public": true,
            "is_default": true
        })),
        Some(&admin_token),
...
async fn test_update_to_default_cache_conflict() {
    ...
    // Create second cache (not default)
    let (status, response) = json_request(
        &server.router,
        "POST",
        "/v1/admin/caches",
