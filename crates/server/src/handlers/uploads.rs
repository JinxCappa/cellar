...
use crate::handlers::resolve_domain_id;
use crate::metrics::{
...
pub async fn create_upload(
    ...
    let domain_id = resolve_domain_id(&state, auth.token.cache_id).await?;
...
mod tests {
    ...
    fn verify_session_ownership_enforces_cache_and_owner() {
        ...
        verify_session_ownership(&auth, &session).unwrap();

        let other_cache = build_session(Some(Uuid::new_v4()), Some(token_id));
        assert!(verify_session_ownership(&auth, &other_cache).is_err());

        let other_owner = build_session(Some(cache_id), Some(Uuid::new_v4()));
        assert!(verify_session_ownership(&auth, &other_owner).is_err());
    }
}
...
pub async fn commit_upload(
    ...
    if session.chunks_predeclared {
        let expected_chunks = state.metadata.get_expected_chunks(upload_id).await?;
