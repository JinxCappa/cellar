...
async fn handle_push_command(
    ...
    let store_paths = if let Some(flake_ref) = flake.as_deref() {
        ...
        resolve_flake_store_paths(flake_ref, &nix_args).await?
    } else {
    ...
    for store_path in store_paths {
        ...
        push_store_path(&http, &profile, &store_path, chunk_size).await?;
    }
...
async fn load_client_config(path: &Path) -> Result<ClientConfig> {
    match tokio::fs::read_to_string(path).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(ClientConfig::default()),
...
async fn push_store_path(
    ...
    cellar_core::store_path::StorePath::parse(store_path)?;
...
async fn push_store_path_inner(
    ...
    let response = http
        .post(format!("{}/v1/uploads", profile.url))
...
async fn fetch_upload_state(
    ...
    let response = http
        .get(format!("{base_url}/v1/uploads/{upload_id}"))
        .bearer_auth(token)
        .send()
        .await?;
...
async fn upload_missing_chunks(
    ...
    for chunk in &nar_index.expected_chunks {
        ...
        in_flight.push(async move {
            let response = client.put(url).bearer_auth(token).body(data).send().await?;
            if response.status().is_success() {
                Ok(())
            } else {
                let status = response.status();
