...
impl S3Backend {
    ...
    pub async fn new(
        ...
        // Apply region if specified, otherwise fall back to us-east-1
        let resolved_region = region.unwrap_or_else(|| "us-east-1".to_string());
        config_loader = config_loader.region(aws_config::Region::new(resolved_region.clone()));
        ...
        // Build S3 client with optional endpoint and path-style settings
        ...
        if let Some(ref endpoint_url) = endpoint {
            ...
            let normalized_endpoint = if endpoint_lower.starts_with("http://")
            ...
            s3_config_builder = s3_config_builder.endpoint_url(&normalized_endpoint);
        }
        ...
        let stored_endpoint = match &endpoint {
            Some(url) => {
...
mod tests {
    ...
    async fn make_backend(prefix: Option<String>) -> S3Backend {
        ...
        let config = aws_sdk_s3::config::Builder::from(&shared).build();
        S3Backend {
            client: Client::from_conf(config),
            bucket: "test-bucket".to_string(),
            prefix,
            endpoint: "http://s3.test".to_string(),
